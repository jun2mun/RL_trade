...
def add_volatility_indicators(high, low, close, n_period=7, fillna=True):
    ...
    df['vol_dc_hband'] = ta.volatility.DonchianChannel(high,low,close, n_period, fillna=fillna).donchian_channel_hband()
    df['vol_dc_lband'] = ta.volatility.DonchianChannel(high,low,close, n_period, fillna=fillna).donchian_channel_lband()
...
def add_volume_indicators(high, low, close, volume, n_period=7, fillna=True):
    ...
    df['volume_cmf'] = ta.volume.ChaikinMoneyFlowIndicator(high, low, close, volume, n_period,
                                                           fillna).chaikin_money_flow()
